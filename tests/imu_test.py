...
def call_heartBeat():
    ...
    while True:
        if enemy_detected:
            CvCmder.CvCmd_Heartbeat(gimbal_pitch_target=0, gimbal_yaw_target=Global_xyz[2], chassis_speed_x=0, chassis_speed_y=0)
        else:
            CvCmder.CvCmd_Heartbeat(gimbal_pitch_target=0, gimbal_yaw_target=Global_xyz[2], chassis_speed_x=0, chassis_speed_y=0)
        time.sleep(1/500)
        ...
        if frame["video"].shape == (480,640,3):
            ...
            for result in results:
                if result != None:
                    ...
                    focal_length_in_pixel =  1280 *0.9* 4.81 / 11.043412
                    focal_length_in_pixel_pitch =  960 * 4.81 / 11.043412
